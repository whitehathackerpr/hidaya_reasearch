"""
Build a professional multi-cell Colab notebook that maps 1:1 to the supervisor's checklist.
"""
import json
...
code([
    ...
    "le = LabelEncoder()\n",
    "y = le.fit_transform(df['cycling_stage'])  # Stages 1-5 → 0-4\n",
...
# ═══════════════════════════════════════════════════════════════════
# CELL: Class Balancing Comparison
# ═══════════════════════════════════════════════════════════════════
md([
...
# ═══════════════════════════════════════════════════════════════════
# CELL: Visualise class distribution
# ═══════════════════════════════════════════════════════════════════
md([
...
code([
    ...
    "        Cls = ExtraTreesClassifier if best_model_name == 'ExtraTrees' else RandomForestClassifier\n",
    "        clf = Cls(\n",
    "            n_estimators=trial.suggest_int('n_estimators', 100, 400),\n",
...
code([
    ...
    "elif best_model_name == 'GradientBoosting':\n",
    "    final_clf = GradientBoostingClassifier(**best_params, random_state=42)\n",
    "else:\n",
    "    Cls = ExtraTreesClassifier if best_model_name == 'ExtraTrees' else RandomForestClassifier\n",
    "    final_clf = Cls(**best_params, random_state=42)\n",
    "\n",
...
# ═══════════════════════════════════════════════════════════════════
md([
    "## §8 — Training vs. Validation Convergence Curves & Overfitting Check\n",
    "\n",
    ...
    "\n",
    "The learning curve shows how F1-Weighted evolves as training data increases. If the training\n",
    "curve plateaus well above the validation curve, it indicates **overfitting** — the gap between\n",
    "the two curves is the *generalisation gap*."
])

code([
    "train_sizes, train_scores, val_scores = learning_curve(\n",
    "    final_pipe, X_train.values, y_train,\n",
    "    cv=StratifiedKFold(n_splits=5, shuffle=True, random_state=42),\n",
    "    n_jobs=1,\n",
    ...
    "plt.title(f'Learning Curve — Generalisation Gap = {gap:.3f}', fontsize=14, fontweight='bold')\n",
    ...
    "\n",
    "if gap > 0.15:\n",
    "    print(f'\\n⚠ HIGH VARIANCE detected (gap={gap:.3f}). The model may be overfitting.')\n",
    "    print('  Regularisation has already been applied via:')\n",
    "    print('  - max_depth constraint (Bayesian-tuned)')\n",
    "    print('  - min_samples_leaf / min_samples_split constraints')\n",
    "    print('  - RFE feature pruning (removing noisy features)')\n",
    "else:\n",
    "    print(f'\\n✓ Generalisation gap is acceptable ({gap:.3f}). No additional regularisation needed.')"
])
...
md([
    ...
    "\n",
    "These are the features that survived RFE elimination, ranked by their Gini importance\n",
    "within the final tuned ensemble model."
])

code([
    "clf_only = final_pipe.named_steps['clf']\n",
    "imp = pd.Series(clf_only.feature_importances_, index=sel_names).sort_values(ascending=True)\n",
    "\n",
    "fig, ax = plt.subplots(figsize=(10, max(5, len(imp) * 0.5)))\n",
    "colors = plt.cm.viridis(np.linspace(0.2, 0.9, len(imp)))\n",
    "imp.plot(kind='barh', ax=ax, color=colors, edgecolor='black', linewidth=0.5)\n",
    "ax.set_title(f'Feature Importance — {best_model_name} (RFE Selected)', fontsize=14, fontweight='bold')\n",
    ...
    "print(f'\\nTop features: {imp_desc.index.tolist()}')"
])
...
notebook = {
    ...
    "metadata": {
        ...
        "colab": {"provenance": [], "name": "Cycling_Stage_Prediction_Pipeline.ipynb"}
    },
    "nbformat": 4,
    "nbformat_minor": 4
}

with open("Cycling_Stage_Prediction_Pipeline.ipynb", "w", encoding="utf-8") as f:
    json.dump(notebook, f, indent=2, ensure_ascii=False)

print("Notebook created: Cycling_Stage_Prediction_Pipeline.ipynb")
